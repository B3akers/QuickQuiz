<script lang="ts">
    ...
    const { questionAnswer, gamePlayers, questionAnswering }: any =
    ...
    const isPlayerAnswer = $derived($questionAnswer?.playerAnswers[answerId]?.includes($session.id));
</script>
...
{#if correctAnswerId >= 0}
    <button
        ...
        class:border={correctAnswerId != answerId && !(playerAnswers?.length > 0)}
        class:border-gray-600={correctAnswerId != answerId && !(playerAnswers?.length > 0)}
        class:bg-red-600={correctAnswerId != answerId && playerAnswers?.length > 0}
